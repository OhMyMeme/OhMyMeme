...
- Preserve the old one-argument `LocalLibraryService.import_paths` call shape when no cancellation event is supplied; QQNT alone passes its worker event so unrelated importers and legacy fakes retain their ABI.

## 2026-08-28 shared catalog query predicates

- Share only ordinary meme WHERE fragments and bound parameters between `MemeDB.search()` and `MemeDB.count()`; keep recent JOIN/order/count and Catalog virtual collection routing outside that builder to preserve behavior.
- Use the `m` table alias in both ordinary query shells so the shared fragments are identical without interpolating user values into SQL.
