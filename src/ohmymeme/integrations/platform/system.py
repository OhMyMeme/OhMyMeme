...
def is_auto_start_enabled() -> bool:
    ...
    system = platform.system()
    if system == "Windows":
        legacy_source_command = "-m " + "src" + ".main --silent"
        try:
            import winreg
            ...
            with winreg.OpenKey(
                ...
                if not getattr(sys, "frozen", False) and legacy_source_command in value:
                    with winreg.OpenKey(
                        winreg.HKEY_CURRENT_USER,
                        key_path,
                        0,
                        ...
                        winreg.SetValueEx(
                            ...
                            winreg.REG_SZ,
                            value.replace(
                                legacy_source_command, "-m ohmymeme --silent"
                            ),
                        )
                return True
