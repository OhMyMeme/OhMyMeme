...
export function useMemes() {
  ...
  async function pasteMemeToChat(id: number): Promise<string> {
    const result = await api('paste_meme_to_chat', id)
    if (!result?.ok) return 'copy_failed'
    return result.paste_status || result.status || 'copied'
  }
  async function setTagbarCollapsed(collapsed: boolean) {
    state.tagbarCollapsed = collapsed
    await api('set_tagbar_collapsed', collapsed)
  }
