...
## 新增功能
- **设置向导** — 首次启动（或旧版本升级后）弹出 7 步设置向导：全局快捷键、开机自启、动图自动播放、云端同步（可跳过）、导入表情（可跳过）；关闭（含 ESC/×/完成）即写入 `meme-index.json` 的 `guide` 标记（重建清单时保留），下次启动不再提示；设置页「基础设置」新增「打开设置向导」按钮可随时重跑
