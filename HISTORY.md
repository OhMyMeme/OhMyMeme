# v0.6.2 — Ohmymeme-AI 文件夹、审核式 AI 与分享包增强

## 新增

- **Wallpaper Engine 风格文件夹主页** — 单层文件夹卡片直接显示在主网格；根目录改为“未归档”视图，已移动进文件夹的表情不会继续出现在根目录。
- **复制/移动的剪切语义** — 拖到文件夹、右键菜单和批量操作均可选择“复制进去”或“移动进去”。复制保留既有归属；移动会在单次数据库事务中移除其他文件夹归属，再写入目标文件夹，并自动补齐同名标签。
- **批量选择扩展** — 支持批量选择表情、文件夹和标签；多选表情可一起拖入收藏夹，也可批量复制/移动、删除或导出。
- **拖拽自动滚动** — 内部拖放和排序拖放靠近列表顶部/底部时自动滚动，支持跨当前视口放入文件夹或排序。
- **分享包界面入口** — 恢复 `.ohmymeme-pack` 导入/导出入口；导出包保存图片、名称、标签、收藏、文件夹归属和排序，不包含密钥、设置或本地路径。
- **AI 审核批量大小** — AI 整理每轮审核数量可在 `1–500` 之间配置，继续坚持“建议生成 → 人工审核 → 确认应用”后才落库。

## 修复与优化

- 修复批量选择模式下单击表情未能即时选中的事件时序问题。
- 修复拖拽预览从窗口左上角跳入的问题，并用 `requestAnimationFrame` 合并预览定位与目标检测，降低拖动抽帧。
- 批量移动改为单个后端桥接调用和原子事务，避免多次调用导致的部分成功、重复刷新或归属残留。

## 文档

- 新增 `docs/ohmymeme-ai-guide.md`，说明 Windows 运行、文件夹语义、批量操作、AI 审核、分享包、受控粘贴和 Android 互通方式。
