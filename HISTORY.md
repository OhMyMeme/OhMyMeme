...
- **macOS 双架构构建** — dmg 文件名带架构后缀 `OhMyMeme-v{version}-{arch}.dmg`，`--arch` 指定 arm64/x86_64（默认自动检测）；`build.yml`/`nightly.yml` 矩阵双架构（arm64 用 macos-latest，x86_64 用 macos-13）；更新检查按本机架构选取对应 dmg
