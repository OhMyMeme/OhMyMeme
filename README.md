...
- **系统托盘运行** — 最小化资源占用，后台常驻
- **全局快捷键** — 默认 `Ctrl+Alt+N` 呼出/隐藏主面板；Windows 可选在鼠标所在屏幕显示隐藏的主面板，默认关闭。内置运行期自愈：回调异常不会拖垮键盘监听线程，且守护线程周期性检测键盘监听线程存活，一旦失效自动重新注册，无需重启软件；热键相关事件（注册/异常/线程死亡/自愈）会追加到 `data_dir/hotkey.log` 便于排查
...
- **右键菜单** — 重命名/收藏/打标签/添加分组/从分组移除/删除；表情卡悬停显示快捷收藏按钮，侧边栏分组悬停显示「⋯」快速操作入口
