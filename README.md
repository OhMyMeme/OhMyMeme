...
- **一键复制** — 点击表情包自动复制到剪贴板（GIF 保留动画）；仅由全局快捷键从隐藏状态呼出的主窗口，在复制成功后自动隐藏
- **原生文件拖拽** — 关闭拖拽排序后可将表情拖到外部应用；仅由全局快捷键从隐藏状态呼出的主窗口，在拖拽成功后自动隐藏
...
| **热键在鼠标处显示** | 默认关闭，仅 Windows 生效；全局热键打开隐藏主面板时按鼠标所在显示器的工作区放置，不影响托盘激活；该次快捷键会话中复制成功或原生文件拖拽成功后自动隐藏 |
