...
- **小尺寸复制** — 主界面「小图」开关：复制超限尺寸（>200px）的静态图时自动转 WebP 缩放到小尺寸，粘贴到 QQ 更接近表情包大小
- **实验性 GIF 隐写** — 设置页「实验性」开关（默认关）：复制超限静态图时用 GIF 隐写，粘贴显示为表情大小，且原图可无损还原；开启后导入含隐写的 GIF 会自动还原原图并一并入库（网格只显示原图并标记「隐写导入」），且会记录隐写文件与原图的哈希关联，下次复制原图时直接复用已有隐写文件、不再重新编码
- **拖放导入** — 直接拖拽图片到窗口即可导入
...
| **实验性：GIF 隐写** | 复制超限静态图时改用 GIF 隐写，可无损还原原图（默认关闭）；导入含隐写的 GIF 自动还原并复用已有隐写文件 |
| **上传进度条** | 上传时显示实时进度弹窗 |
| **上传完毕提示** | 上传完成后弹窗告知结果 |
| **下载进度条** | 下载时显示实时进度弹窗 |
| **下载完毕提示** | 下载完成后弹窗告知结果 |
...
- 已集成到设置页「从电脑导入」向导：环境检查 → 选账号 → 输出位置 → 进度与结果汇总；手动选择的配置文件/用户数据目录会持久化，提取过程支持取消
...
### 静态图缩放复制
主界面「小图」开关（配置 `copy_resize_enabled`，默认开）：复制超过 `copy_resize_max`（默认 200px）的静态图时，用 PIL 转成缩放到限制内的 WebP 再写入剪贴板（QQ/微信原生支持 WebP）；动图（GIF/动画 WebP）不受影响。重采样结果写入系统临时目录 `ohmm_resize_<md5>_<max>_q<质量>_v<版本>.webp` 并保留（CF_HDROP 需在粘贴时仍可读取）；缓存键含编码参数与版本号，改编码逻辑后旧缓存自动失效，命中时校验文件完整性，同一表情重复复制直接复用。
