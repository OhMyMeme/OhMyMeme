...
- **微信表情导入** — 从微信电脑版缓存提取收藏表情（Windows，需微信正在运行）：C++ 辅助二进制提取进程内存密钥 → AES-CBC 解密表情数据库 → CDN 下载并校验 MD5；支持多账号选择，二进制发布时校验 SHA-256 防篡改
...
| **微信导入** | 从微信电脑版缓存提取收藏表情（Windows，需微信运行）：辅助二进制提取密钥 → 解密数据库 → CDN 下载校验；支持多账号选择 |
